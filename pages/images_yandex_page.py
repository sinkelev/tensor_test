...
class ImagesPage(Region):
    ...
    images_page_inp   =   TextField(    By.CSS_SELECTOR, '.input__control', 'Поле ввода')
    ...
    image_elm         =   Element(      By.CSS_SELECTOR, '.MMImage-Preview', 'Картинка в карусели')
    ...
    def open_and_test_first_category(self):
        ...
        input_txt = self.images_page_inp.get_attribute('value')
        assert_that(category_txt, equal_to(input_txt), 'Имя категории не совпадает с запросом')
    ...
    def open_and_check_first_image(self):
        ...
        self.image_elm.should_be(Displayed)

    def go_on_next_and_prev_images(self):
        """Переключение картинок кнопками навигации и проверка"""
        first_image_src = self.image_elm.get_attribute('src')
        self.next_btn.click()
        next_image_src = self.image_elm.get_attribute('src')
        assert_that(first_image_src, not_equal(next_image_src), 'Картинки одинаковые')
        self.prev_btn.click()
        last_image_src = self.image_elm.get_attribute('src')
        assert_that(first_image_src, equal_to(last_image_src), 'Картинки не одинаковые')
